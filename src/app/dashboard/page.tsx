 ...
 * Dashboard Page
 * 
 * Protected page that shows user information and database sync status.
...
export default async function DashboardPage() {
  try {
    ...
    // If Clerk is not configured or auth fails, show error
    return (
      <div className="flex min-h-screen items-center justify-center bg-zinc-50 dark:bg-black">
        <main className="flex w-full max-w-md flex-col items-center gap-8 rounded-2xl bg-white p-8 shadow-lg dark:bg-zinc-900">
          <h1 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-50">
          ...
          <p className="text-center text-sm text-zinc-600 dark:text-zinc-400">
            {error instanceof Error ? error.message : 'Authentication is not configured. Please set up Clerk environment variables.'}
          </p>
          <Link
            href="/"
            className="flex h-10 items-center justify-center rounded-full border border-zinc-200 px-6 text-sm font-medium text-zinc-900 transition-colors hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-50 dark:hover:bg-zinc-800"
          >
            ← Back to Home
          </Link>
        </main>
